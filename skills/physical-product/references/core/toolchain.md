...
|---|---|---|
| Chrome (SVG→PNG, viz renders — needs a GPU) | app bundle | `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome` |
...
| Chrome for viz | photoreal viz (use CAD screenshots) | No photoreal visualization — Chrome (the render engine's host) is unavailable; technical CAD renders are attached instead. |
...
├── viz/               # render engine (index.html + render.bundle.js, from the pack)
