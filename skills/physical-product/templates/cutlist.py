"""Cutting plan (nářezový plán) — which stock to buy and how to cut it.

- STOCK maps a Czech stock name -> Bar (1D) or Sheet (2D) with usable size,
...
- Piece sizes come from part bounding boxes (longest axis for bars, two
  largest axes for sheets); explicit overrides cover miters and rozviny.
...
Outputs (uv run cutlist.py):
    out/cutlist.md    Czech cut tables + purchasing summary (+ stdout)
...
class Extra:
    ...
    label: str                          # Czech, shown in tables/SVG
...
@dataclass(frozen=True)
class Bar:
    """1D stock (tyč, jäkl, trubka, lišta)."""
    ...
    price: float | None = None      # Kč / stock unit (purchasing summary)


@dataclass(frozen=True)
class Sheet:
    """2D stock (deska, spárovka, překližka). width = grain direction."""
    width: float
    height: float
    kerf: float = 4.0
...
# a size override (the bracket is bent plech — its flat rozvin, not bbox),
# Extra pieces, and the thickness/profile sanity declarations.
# --------------------------------------------------------------------------
STOCK: dict[str, Bar | Sheet] = {
    "plech 5 mm": Sheet(width=600, height=200, kerf=2.0, thickness=5.0,
                        parts=(("bracket", (175.0, 60.0)),)),
    "jäkl 20×20×2": Bar(length=6000, kerf=3.0, profile=(20, 20),
                        parts=(Extra("vzpěra (ukázka)", 500.0, count=4),)),
}

# Parts deliberately NOT cut from stock (3D print, purchased finished):
# key -> Czech reason; listed in the md, suppresses the coverage warning.
NO_STOCK: dict[str, str] = {}


def group_keys(name: str) -> list[str]:
    """All PARTS keys whose spec.group == name — declare a 25-lamella
    family as parts=(*group_keys("lamela"),)."""
...
def expand_pieces(name: str, stock: Bar | Sheet,
    ...
    for ref in stock.parts:
        if isinstance(ref, Extra):
            ...
            covered.add(key)
            label, count = spec.czech_name, spec.count
                ...
                if override is None:
                    ...
                    if stock.thickness and abs(
                        ...
                        print(f"WARNING: {key} thickness {dims[2]:g} != "
                              ...
                              "(key, (w, h)) override if intended (rozvin?)",
                              file=sys.stderr)
...
def render_markdown(plans: list[StockPlan], uncovered: list[str]) -> str:
    lines = ["# Nářezový plán", ""]
    if not plans:
        lines += ["Žádný polotovar nedeklarován (STOCK je prázdný).", ""]
    for plan in plans:
        st = plan.stock
        if isinstance(st, Bar):
            lines += [f"## {plan.name} — tyč {_mm(st.length)} mm, "
                      f"šířka řezu {_mm(st.kerf)} mm", ""]
            for i, u in enumerate(plan.units, 1):
                rest = st.length - u.used
                lines += [f"### Tyč {i}/{len(plan.units)} — obsazeno "
                          f"{_mm(u.used)} mm, zbytek {_mm(rest)} mm", "",
                          "| Řez | Díl | Délka (mm) | Ks |", "|---|---|---|---|"]
            ...
            lines += [f"## {plan.name} — deska {_mm(st.width)} × "
                      f"{_mm(st.height)} mm, šířka řezu {_mm(st.kerf)} mm",
                      ""]
            for i, u in enumerate(plan.units, 1):
                net = sum(pl.w * pl.h for pl in u.placements)
                util = net / (st.width * st.height)
                lines += [f"### Deska {i}/{len(plan.units)} — využití "
                          f"{util:.0%}", "",
                          "| Pás | Díl | Rozměr (mm) | Ks |",
                          "|---|---|---|---|"]
                rows = []
    ...
    if plans:
        ...
        head = "| Materiál | Polotovar | Nakoupit | Využití | Největší zbytek |"
        sep = "|---|---|---|---|---|"
        if has_price:
            head += " Cena |"
            sep += "---|"
        lines += ["## Nákupní souhrn", "", head, sep]
        for name, kind, n, util, offcut, price in purchase_rows(plans):
            row = f"| {name} | {kind} | {n} ks | {util:.0%} | {offcut} |"
            if has_price:
                row += (f" {_mm(price * n)} Kč |" if price else " — |")
            lines.append(row)
        lines.append("")
    if NO_STOCK or uncovered:
        lines += ["## Díly mimo nářezový plán", "", "| Díl | Důvod |",
                  "|---|---|"]
        for key, reason in NO_STOCK.items():
            lines.append(f"| {model.PARTS[key].czech_name} | {reason} |")
        for key in uncovered:
            lines.append(f"| {model.PARTS[key].czech_name} | "
                         "— CHYBÍ V PLÁNU |")
        lines.append("")
    lines += ["Plán je deterministický odhad (first-fit) — skutečná "
              "spotřeba je horní mez; profesionální nesting může ušetřit "
              "kus. Řezy jsou vedeny jako průběžné (gilotinové); využití "
              "= čistá plocha/délka dílů ku nakoupenému materiálu.", ""]
    return "\n".join(lines)


def purchase_rows(plans: list[StockPlan] | None = None):
    """(materiál, polotovar, ks, využití, největší zbytek, price|None) —
    for the buildsheet generator (mirror of the Nákupní souhrn table)."""
    if plans is None:
        plans, _ = plan_all()
    rows = []
    for p in plans:
        st = p.stock
        kind = (f"tyč {_mm(st.length)} mm" if isinstance(st, Bar)
                else f"deska {_mm(st.width)} × {_mm(st.height)} mm")
...
def render_svg(plans: list[StockPlan]) -> tuple[str, int, int]:
    ...
    body: list[str] = []
    y = _MARGIN
    body.append(_svg_text(_MARGIN, y + 14, "Nářezový plán", 18, "start"))
    y += 34
    ...
    for plan in plans:
        ...
        for i, u in enumerate(plan.units, 1):
            if isinstance(st, Bar):
                title = (f"{plan.name} — tyč {i}/{len(plan.units)} "
                         f"({_mm(st.length)} mm)")
            else:
                title = (f"{plan.name} — deska {i}/{len(plan.units)} "
            ...
            if isinstance(st, Bar):
                ...
                if rest * scale > 60:
                    body.append(_svg_text(
                        ...
                        y + h_px / 2 + 4, f"zbytek {_mm(rest)}", 12,
                        color="#666"))
                y += h_px + 6
            else:
